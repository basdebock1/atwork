<x-app-layout>
    <div class="admin" style="gap: 0">
        <form class="admin-form" action="/nieuws/{{ $article->id }}" method="POST" enctype="multipart/form-data">
            <h2>Pas bericht aan</h2>
            @csrf
            @method('PUT')
            ...
            <button class="btn blue">Nieuwe gegevens opslaan</button>
        </form>
        <form class="admin-form" action="/nieuws/{{ $article->id }}" method="POST">
            @csrf
            @method('DELETE')
            <button class="btn red">Verwijder bericht</button>
        </form>
    </div>
</x-app-layout>
